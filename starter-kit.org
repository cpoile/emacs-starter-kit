...
And we have to setq yas-snippet-dirs early because packages autoloaded in the starter-kit-elpa.org file will use it.
...
*** Starter kit yasnippet (not tangled - defined above)
load the yasnippet bundle in [[file:starter-kit-yasnippet.org][starter-kit-snippet]]
#+header: :tangle no
#+begin_src emacs-lisp
...
*** Starter Kit Python (not tangled)
